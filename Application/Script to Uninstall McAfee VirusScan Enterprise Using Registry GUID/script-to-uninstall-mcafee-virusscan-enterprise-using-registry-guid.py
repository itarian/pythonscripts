...
def reg():
    ...
    blacklist=r'McAfee VirusScan Enterprise'
...
def uninstall (find):
	...
	time.sleep(120)
	inst=check()
	find=re.findall('{.*}\s\sMcAfee\sVirusScan\sEnterprise',inst)
	if len(find) >0:
		print "McAfee VirusScan Enterprise is not uninstalled on Endpoint"
...
if len(inst)>0:
    ...
    if len(find)>0:
        ...
        if len(final) == 38:
            ...
            print "McAfee VirusScan Enterprise is installed on Endpoint"
        ...
        print "McAfee VirusScan Enterprise is not installed at End point"
